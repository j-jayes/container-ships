...
```{r}
df <- read_rds("data/vessel_finder_data_detailed_clean.rds")
```
...
umap_rec <- recipe( ~ ., data = df_type_6) %>% 
  update_role(name, type, new_role = "id") %>% 
  step_normalize(all_predictors()) %>% 
...
```{r}
umap_rec_3d <- recipe( ~ ., data = df_type_6) %>% 
  update_role(name, type, new_role = "id") %>% 
  step_normalize(all_predictors()) %>% 
  step_umap(all_predictors(), num_comp = 3)
...
tsne_rec <- recipe( ~ ., data = df_type_6 %>% select(launched:width)) %>% 
  # update_role(name, type, new_role = "id") %>% 
  step_normalize(all_predictors())

df_type_6_matrix <- df_type_6 %>% select(launched:width) %>%
  distinct() %>% 
  as.matrix() %>% scale()

df_type_6_labels <- df_type_6 %>%
  distinct(across(launched:width), .keep_all = T) %>% select(name, type)
